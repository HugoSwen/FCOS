...
class FCOS(nn.Module):
    ...
    def train(self, mode=True):
        '''
        set module train mode, and frozen bn
        '''
...
class FCOSDetector(nn.Module):
    ...
    def forward(self, inputs):
        '''
        inputs 
        [train] list  batch_imgs,batch_boxes,batch_classes
        [predict] img
        '''
        ...
        if self.mode == "train":
            ...
            # raise NotImplementedError("no implement predict model")
            '''
            for predict mode, img should preprocessed before feeding in net 
            '''
            batch_imgs = inputs
            out = self.fcos_body(batch_imgs)
